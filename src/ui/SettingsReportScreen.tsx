...
function isSettingsReportMode(
  value: string | undefined,
): value is SettingsReportMode {
...
function yearRows(
  ...
  for (const transaction of transactions) {
    ...
    totals[Number(date.slice(5, 7)) - 1] += amountForMetric(
      transaction,
      metric,
    );
  }
...
export function SettingsReportScreen() {
  ...
  const [year, setYear] = useState(() =>
    Number(todayVietnamDate().slice(0, 4)),
  );
  ...
  const periodRows = useMemo(
    () =>
      scope === "all"
        ? allTimeRows(transactions, metric)
        : yearRows(transactions, year, metric, locale),
    [locale, metric, scope, transactions, year],
  );
  ...
  return (
    <div className="space-y-4 px-4 py-5 text-slate-100">
      ...
      <div className="grid min-h-14 grid-cols-[2.75rem_minmax(0,1fr)_2.75rem] items-center gap-2 rounded-2xl border border-white/10 bg-white/[0.07] px-2">
        ...
        <div className="text-center">
          <div className="text-xl font-bold text-white">
            {scope === "year" ? year : t("reports.allTime")}
          </div>
          <div className="text-xs font-medium text-slate-400">
            {formatPeriodLabel(mode, year, t)}
          </div>
        </div>
        {scope === "year" ? (
          <button
      ...
      {!report.loading && !report.error && (
        <>
          <section
            aria-label={t("reports.totals")}
            className="grid grid-cols-3 gap-2"
          >
            <MetricCard
              label={t("reports.expenseTotal")}
              value={formatVND(totals.expense, locale)}
              tone="expense"
            />
            <MetricCard
              label={t("reports.incomeTotal")}
              value={formatVND(totals.income, locale)}
              tone="income"
            />
            <MetricCard
              label={t("reports.netTotal")}
              value={formatVND(totals.net, locale)}
              tone="neutral"
            />
          </section>
