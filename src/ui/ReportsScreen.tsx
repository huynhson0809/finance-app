...
import {
  useScopedReportTransactions,
  type ScopedReportKind,
} from "../hooks/useScopedReportTransactions";
import {
  categoryDayTotals,
  categorySummaries,
  totalsByDirection,
...
function safeMonth(value: string | null): string {
  return value && VALID_MONTH.test(value)
    ? value
    : monthOfVietnamDate(todayVietnamDate());
}
...
function signedAmount(transaction: Transaction): number {
  return transactionDirection(transaction) === "income"
    ? transaction.amount
    : -transaction.amount;
}
...
function scopeForReportMode(mode: ReportMode | null): ScopedReportKind {
  ...
  if (mode === "all-summary" || mode === "all-category" || mode === "search")
    return "all";
  return "year";
}
...
function yearRowsForAllTime(
  ...
  for (const transaction of transactions) {
    const year = Number(
      todayVietnamDate(new Date(transaction.occurredAt)).slice(0, 4),
    );
    totals.set(
      year,
      (totals.get(year) ?? 0) + amountForMetric(transaction, metric),
    );
  }
...
export function ReportsScreen() {
  ...
  const direction: TransactionDirection =
    searchParams.get("direction") === "income" ? "income" : "expense";
  ...
  const reportTransactions = reportScope
    ? scopedReport.transactions
    : transactions;
  ...
  const isPeriodSummaryReport =
    reportMode === "year-summary" || reportMode === "balance-change";
  ...
  const noDirectionDataLabel = t("reports.noDirectionData", {
  ...
  const categoryRows = useMemo(() => {
    ...
    for (const transaction of reportTransactions) {
      if (transactionDirection(transaction) !== direction) continue;
      if (!categoryBelongsToDirection(transaction.category, direction))
        continue;
    ...
    const directionTotal =
      ...
      Array.from(customTotals.values()).reduce(
        (total, value) => total + value,
        0,
      );
    const customRows: CategorySummary[] = Array.from(
      customTotals,
      ([category, total]) => ({
        category,
        direction,
  ...
  const periodMetric: PeriodMetric =
    reportMode === "balance-change" ? "net" : direction;
  ...
  const periodAverage =
    periodRows.length > 0 ? Math.round(periodTotal / periodRows.length) : 0;

  const effectiveCategory =
    selectedCategory && categoryBelongsToDirection(selectedCategory, direction)
      ? selectedCategory
      : null;
  ...
  const searchTransactions = useMemo(() => {
    ...
    const newestFirst = [...reportTransactions].sort(
      (a, b) =>
    ...
    return newestFirst.filter((transaction) => {
      ...
      const values = [
        ...
        formatMoney(signedAmount(transaction), transaction.currency, locale),
        transaction.category,
        label,
      ];
        ...
        <>
          {!reportScope && (
            <BudgetAlert
              ...
              categoryLabel={(c) =>
                categoryLabel(c, customCategories, t, categoryOverrides)
              }
            />
          )}
              ...
              <GlassPanel className="overflow-hidden">
                ...
                {reportMode !== "balance-change" && (
                  <div className="grid min-h-14 grid-cols-[minmax(0,1fr)_auto] items-center gap-3 border-b border-white/10 px-4">
                    ...
                    <span className="text-lg font-bold text-sky-300">
                      {formatVND(periodAverage, locale)}
                    </span>
                  </div>
                )}
                {periodRows.map((row) => (
          ...
          {!isSummaryReport && effectiveCategory ? (
            <section className="space-y-4">
              ...
              <GlassPanel className="mx-4 p-3">
                {reportScope === "year" ? (
                  <PeriodBar
                    data={detailPeriodRows}
                    color={direction === "income" ? "#34d399" : "#fb7185"}
                  />
                ) : (
