...
import { saveAssetTransfer, saveTransactionWithAssetEffect } from '../assets/save';
...
function defaultAccountId(accounts: readonly AssetAccount[]): string {
  return accounts.find(account => account.kind === 'cash' || account.kind === 'bank')?.id
    ?? accounts[0]?.id
    ?? '';
}
...
export function AddScreen() {
  ...
  const labels = isEnglish
    ? {
        ...
        differentAccounts: 'The source and destination accounts must be different.',
  ...
  const categoryDirection: TransactionDirection = mode === 'income' ? 'income' : 'expense';
  ...
  const transactionAccounts = mode === 'income' ? incomeAccounts : expenseAccounts;
  ...
  const amountCurrency = mode === 'transfer'
    ? selectedTransferSource?.currency ?? null
    : 'VND';
  const previousAmountCurrencyRef = useRef<'VND' | 'USD' | null>(amountCurrency);
  const transferDestinationAccounts = useMemo(
    () => transferDestinationBase.filter(
      account => !selectedTransferSource || account.currency === selectedTransferSource.currency,
    ),
  ...
  useEffect(() => {
    setToAccountId(current => {
      ...
      return defaultAccountId(
        transferDestinationAccounts.filter(account => account.id !== fromAccountId),
      );
    });
  ...
  useEffect(() => {
    ...
    if (previousCurrency && amountCurrency && previousCurrency !== amountCurrency) {
      setRaw('');
    }
  ...
  function handleAmountChange(value: string) {
    if (mode === 'transfer' && selectedTransferSource?.currency === 'USD') {
  ...
  function handleMode(next: AddMode) {
    ...
    const nextDirection: TransactionDirection = next === 'income' ? 'income' : 'expense';
    setMode(next);
    setSaveError(null);
  ...
  const parsedAmount = mode === 'transfer' && selectedTransferSource?.currency === 'USD'
    ? parseUsdInput(raw)
    : parseVNDInput(raw);
  ...
  const transactionAccountValid = !accountsQuery.isLoading && !accountsQuery.isError && (
  ...
  if (mode === 'transfer' && !accountsQuery.isLoading && !accountsQuery.isError) {
    ...
    } else if (selectedTransferSource.currency !== selectedTransferDestination.currency) {
  ...
  const hasTransferSelectors = transferSourceAccounts.length > 0
    || transferDestinationBase.length > 0;
  const needsAnotherTransferAccount = transferSourceAccounts.length === 0
    || !transferDestinationAccounts.some(account => account.id !== fromAccountId);
    ...
    >
      ...
      <div data-testid="add-fixed-form" className="shrink-0 space-y-4">
        ...
        {accountsQuery.isError ? (
          ...
          <AccountSelect
            label={mode === 'expense' ? labels.expenseAccount : labels.incomeAccount}
            placeholder={labels.chooseAccount}
        ...
        {mode === 'transfer' && hasTransferSelectors && needsAnotherTransferAccount && (
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <DarkField label={t('add.amount')}>
            <input
              ...
              inputMode={mode === 'transfer' && selectedTransferSource?.currency === 'USD'
                ? 'decimal'
                : 'numeric'}
              pattern={mode === 'transfer' && selectedTransferSource?.currency === 'USD'
                ? '[0-9]+([.,][0-9]{0,2})?'
                : '[0-9]*'}
            />
          </DarkField>
        ...
        >
          {saving
            ...
            : mode === 'transfer'
              ...
              : t(mode === 'expense' ? 'add.submitExpense' : 'add.submitIncome')}
        </button>
      </footer>
    </div>
  );
