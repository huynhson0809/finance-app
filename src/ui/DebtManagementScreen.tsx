...
import { MoneyInputField } from './components/MoneyNumpad';
...
export function DebtManagementScreen() {
  ...
  const { debts, loading, error, addDebt, editDebt, removeDebt } = useDebts();
  ...
  const lentDebts = useMemo(() => debts.filter(d => d.direction === 'lent'), [debts]);
  const borrowedDebts = useMemo(() => debts.filter(d => d.direction === 'borrowed'), [debts]);
  const totalLentRemaining = useMemo(
    () => lentDebts.filter(d => !d.settled).reduce((s, d) => s + d.totalAmount, 0),
    [lentDebts],
  );
  const totalBorrowedRemaining = useMemo(
    () => borrowedDebts.filter(d => !d.settled).reduce((s, d) => s + d.totalAmount, 0),
    [borrowedDebts],
  );
  ...
  function renderDebtRow(debt: Debt) {
    return (
      <button
        ...
        className={`grid w-full grid-cols-[minmax(0,1fr)_minmax(5rem,7.5rem)_1.25rem] items-center gap-2 border-b border-white/10 px-4 py-3 text-left last:border-b-0 ${
      ...
      >
        <span className="min-w-0">
          ...
          {debt.note && (
            <span className="block truncate text-xs text-slate-400">{debt.note}</span>
          )}
          {debt.settled && (
            <span className="text-xs font-semibold text-emerald-400">{t('debts.settled')}</span>
          )}
        </span>
  ...
  return (
    <div className="space-y-4 px-4 py-5 pb-36 text-slate-100">
      <header className="grid grid-cols-[2.75rem_minmax(0,1fr)_2.75rem] items-center gap-2">
        ...
        <h1 className="truncate text-center text-xl font-bold text-white">{t('debts.title')}</h1>
        <button
          type="button"
          onClick={startNew}
      ...
      <div className="grid grid-cols-2 gap-3">
        <GlassPanel className="p-3">
          ...
          <p className="mt-1 text-lg font-bold text-sky-400">{formatVND(totalLentRemaining, locale)}</p>
        </GlassPanel>
        <GlassPanel className="p-3">
          <p className="text-xs text-slate-400">{t('debts.totalBorrowed')}</p>
          <p className="mt-1 text-lg font-bold text-rose-400">{formatVND(totalBorrowedRemaining, locale)}</p>
        </GlassPanel>
      </div>
      ...
      {loading ? (
        ...
        <>
          {lentDebts.length > 0 && (
            <section>
              <h2 className="mb-2 text-sm font-semibold text-slate-400">{t('debts.lent')}</h2>
          ...
          {borrowedDebts.length > 0 && (
            <section>
              <h2 className="mb-2 text-sm font-semibold text-slate-400">{t('debts.borrowed')}</h2>
              <GlassPanel className="overflow-hidden">
...
function DebtDetailView({
  ...
  const { payments, loading, addPayment, removePayment } = useDebtPayments(debt.id);
  ...
  const progress = debt.totalAmount > 0 ? Math.min(paidTotal / debt.totalAmount, 1) : 0;
  ...
  return (
    <div className="space-y-4 px-4 py-5 pb-36 text-slate-100">
      <header className="grid grid-cols-[2.75rem_minmax(0,1fr)_2.75rem] items-center gap-2">
        ...
        <h1 className="truncate text-center text-xl font-bold text-white">{debt.personName}</h1>
      ...
      <GlassPanel className="p-4">
        <div className="flex items-baseline justify-between gap-3">
          ...
          <span className={`text-2xl font-bold ${debt.direction === 'lent' ? 'text-sky-400' : 'text-rose-400'}`}>
            {formatVND(debt.totalAmount, locale)}
          </span>
        </div>
        {debt.note && (
        ...
        <div className="mt-4">
          <div className="flex justify-between text-xs text-slate-400">
            <span>{t('debts.paid')}: {formatVND(paidTotal, locale)}</span>
      ...
      <section>
        <h2 className="mb-2 text-sm font-semibold text-slate-400">{t('debts.payments')}</h2>
        {loading ? (
          <GlassPanel className="p-4 text-center text-sm text-slate-400">Loading...</GlassPanel>
        ) : payments.length === 0 ? (
