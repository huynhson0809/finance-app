  ...
  const todayExpense = useMemo(
    () =>
      monthTx
        .filter(
          (tx) =>
            tx.direction !== "income" && isSameVietnamDay(tx.occurredAt, today),
        )
    ...
    <div className="min-h-screen bg-black pb-28 text-zinc-50">
      ...
      >
        <SummaryCell
          ...
          value={monthValue(monthTotals.income)}
          tone="income"
        />
        <SummaryCell
          label={t("home.monthExpense")}
          value={monthValue(monthTotals.expense)}
          tone="expense"
        />
        <SummaryCell
        ...
        />
        <SummaryCell
          label={t("home.todaySpend")}
          value={monthValue(todayExpense)}
          tone="expense"
        />
        <SummaryCell
          label={t("home.todayIncome")}
          value={monthValue(todayIncome)}
          tone="income"
        />
      ...
      <div className="grid grid-cols-2 gap-2 px-3 pb-3">
        <Link
          to="/add"
          className="flex min-h-16 flex-col items-center justify-center rounded-lg border border-white/10 bg-zinc-900 text-sm font-semibold text-sky-400"
        >
          {t("nav.add")}
        </Link>
      ...
      {cloudErrors.length > 0 && (
        <div
          role="alert"
          className="mx-3 mb-3 rounded-lg border border-rose-300/30 bg-rose-500/10 p-3 text-sm text-rose-100"
        >
          <div>{cloudErrors.join(" ")}</div>
          <button
            type="button"
            className="mt-2 rounded-lg bg-rose-400 px-3 py-2 font-semibold text-slate-950"
