...
function typeLabelKey(transaction: Transaction): string {
  ...
  if (transaction.bank === 'ACB' && transaction.transactionType === 'balance_alert') {
...
function sourceLabelKey(transaction: Transaction): string {
  ...
  if (transaction.source === 'bank-screenshot') return 'transactionEdit.sourceBankScreenshot';
...
export function TransactionEditScreen() {
  ...
  if (loading) {
    return <div className="px-4 py-5 text-sm text-slate-300">{t('transactionEdit.loading')}</div>;
  }
  ...
  return (
    <div className="space-y-4 px-4 py-5">
      <header className="flex items-center gap-3">
        ...
        <h1 className="text-2xl font-bold text-white">{t('transactionEdit.title')}</h1>
      </header>
      ...
      <GlassPanel className="p-4">
        <dl className="grid grid-cols-3 gap-3">
          <MetadataItem label={t('transactionEdit.source')} value={t(sourceLabelKey(transaction))} />
          <MetadataItem label={t('transactionEdit.bank')} value={transaction.bank ?? '-'} />
          <MetadataItem label={t('transactionEdit.type')} value={t(typeLabelKey(transaction))} />
        </dl>
      </GlassPanel>
