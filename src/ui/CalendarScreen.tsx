...
function transactionAmount(transaction: Transaction, locale: 'en' | 'vi'): string {
...
function buildCalendarCells(days: CalendarDaySummary[], monthISO: string): CalendarCell[] {
  ...
  const leading: CalendarCell[] = Array.from({ length: leadingCount }, (_, index) => ({
    kind: 'adjacent',
    date: dateForMonthDay(previousMonth, previousMonthDays - leadingCount + index + 1),
  }));
  ...
  const trailing: CalendarCell[] = Array.from({ length: trailingCount }, (_, index) => ({
    kind: 'adjacent',
        ...
        {cells.map(cell => {
          if (cell.kind === 'adjacent') {
            ...
            return (
              ...
              >
                <span className={weekday === 5 ? 'text-sky-800' : weekday === 6 ? 'text-red-900' : ''}>
                  {dayNumber(cell.date)}
                </span>
              </div>
            );
...
function MonthSummary({
  ...
  return (
    <section aria-label={t('calendar.monthSummary')} className="grid grid-cols-3 bg-black px-4 py-3">
      <div className="text-center">
        <div className="text-xs font-semibold text-zinc-300">{t('calendar.income')}</div>
        <div className="mt-0.5 text-sm font-bold text-sky-500">{formatVND(income, locale)}</div>
      </div>
      <div className="text-center">
        <div className="text-xs font-semibold text-zinc-300">{t('calendar.expense')}</div>
        <div className="mt-0.5 text-sm font-bold text-red-500">{formatVND(expense, locale)}</div>
      </div>
      <div className="text-center">
        <div className="text-xs font-semibold text-zinc-300">{t('calendar.total')}</div>
        <div className={['mt-0.5 text-sm font-bold', net >= 0 ? 'text-sky-500' : 'text-red-500'].join(' ')}>
...
function CalendarLedger({
  ...
  if (!hasMonthTransactions) {
    return <div className="px-4 py-4 text-sm text-zinc-400">{t('calendar.emptyMonth')}</div>;
  }
    ...
    <section aria-label={t('calendar.dateGroups')} className="bg-black">
      <article aria-label={dateGroupTitle(selectedDate, locale)}>
        ...
        {transactions.length === 0 ? (
          <div className="px-4 py-4 text-sm text-zinc-400">{t('calendar.emptyDay')}</div>
        ) : (
          <ul>
                  ...
                  >
                    ...
                    <span
                      className={[
                        ...
                        transaction.direction === 'income' ? 'text-sky-500' : 'text-zinc-50',
                      ].join(' ')}
                    >
                      {amount}
                    </span>
...
function CalendarMonthView({ month, today, locale }: CalendarMonthViewProps) {
  ...
  const hasMonthTransactions = transactions.some(
    transaction => todayVietnamDate(new Date(transaction.occurredAt)).slice(0, 7) === month,
  );
    ...
    <div className="pb-24 text-slate-100">
      ...
      <div className="grid grid-cols-[2.5rem_minmax(0,1fr)_2.5rem] items-center gap-2 bg-[#202020] px-2 py-3">
        ...
        <div className="rounded-md bg-zinc-800 px-3 py-2 text-center">
          <span className="text-lg font-bold text-white">{displayMonth(month)}</span>
