...
const ENTRIES: Array<Omit<CategoryRule, "id">> = [
  ...
  seed("coopmart", "food-drinks"),
  seed("coop food", "food-drinks"),
  seed("co.op food", "food-drinks"),
...
const EMAIL_RULES = SEED_RULES.filter(
  (rule) => !EMAIL_EXCLUDED_PATTERNS.has(rule.pattern),
);
