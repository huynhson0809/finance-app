...
function resetMocks() {
  ...
  assetMocks.invokeFunction.mockResolvedValue({
    data: { ok: true },
    error: null,
  });
  assetMocks.supabase = {
  ...
  assetMocks.listCloudAssetAccounts.mockImplementation(() =>
    Promise.resolve([...assetMocks.accounts]),
  );
  assetMocks.listCloudAssetRates.mockImplementation(() =>
    Promise.resolve([...assetMocks.rates]),
  );
  assetMocks.listCloudAssetEvents.mockResolvedValue([]);
  assetMocks.upsertCloudAssetAccount.mockImplementation(
    async (_client: unknown, input: Partial<AssetAccount>) => {
      ...
      const saved = account({
        ...
        sortOrder:
          input.sortOrder ?? existing?.sortOrder ?? assetMocks.accounts.length,
        createdAt: input.createdAt ?? existing?.createdAt ?? nowIso(),
        updatedAt: nowIso(),
      });
      const existingIndex = assetMocks.accounts.findIndex(
      ...
      assetMocks.accounts.sort(
        (left, right) => left.sortOrder - right.sortOrder,
      );
      return saved;
    },
  );
  assetMocks.reorderCloudAssetAccounts.mockImplementation(
    async (_client: unknown, ids: string[]) => {
  ...
  assetMocks.upsertCloudAssetRate.mockImplementation(
    async (_client: unknown, input: Pick<AssetRate, "pair" | "value">) => {
      const existing = assetMocks.rates.find(
        (item) =>
          item.pair === input.pair &&
          item.source === "manual" &&
          item.userId === "user-1",
      );
      const saved = rate({
  ...
  assetMocks.deleteCloudAssetRate.mockImplementation(
    async (_client: unknown, pair: AssetRate["pair"]) => {
      assetMocks.rates = assetMocks.rates.filter(
        (item) =>
          !(
            item.pair === pair &&
            item.source === "manual" &&
  ...
  assetMocks.refreshCloudAssetRates.mockImplementation(async () => {
    assetMocks.rates = assetMocks.rates.map((item) =>
      item.source === "auto"
        ? {
            ...item,
...
describe("AssetManagementScreen", () => {
  ...
  it("shows effective values, sources, and fetched times for both rate pairs", async () => {
    ...
    expect(
      await within(usdRate).findByText("1 USD = 24.500 ₫"),
    ).toBeInTheDocument();
    ...
    expect(
      within(usdRate).getByRole("button", { name: /Xóa.*USD.*VND/ }),
    ).toBeDisabled();

    const goldRate = screen.getByRole("region", { name: "Gram vàng / VND" });
    expect(
      within(goldRate).getByText("1 gram vàng = 2.100.000 ₫"),
    ).toBeInTheDocument();
    ...
    expect(
      within(goldRate).getByRole("button", {
        name: /Xóa.*Gram.*VND/,
      }),
    ).toBeEnabled();
  });
  ...
  it("saves a Vietnamese-formatted override and clears it back to automatic", async () => {
    assetMocks.rates = assetMocks.rates.filter(
      (item) => !(item.pair === "USD_VND" && item.source === "manual"),
    );
    ...
    await user.click(
      within(usdRate).getByRole("button", {
        name: /Lưu.*USD.*VND/,
      }),
    );

    await waitFor(() => {
      expect(assetMocks.upsertCloudAssetRate).toHaveBeenCalledWith(
        assetMocks.supabase,
        {
          pair: "USD_VND",
          value: 25_500.5,
    ...
    expect(
      await within(usdRate).findByText("1 USD = 25.500,5 ₫"),
    ).toBeInTheDocument();
    ...
    await waitFor(() => {
      expect(assetMocks.deleteCloudAssetRate).toHaveBeenCalledWith(
        assetMocks.supabase,
        "USD_VND",
      );
    });
  ...
  it("shows refresh progress and all-refreshed feedback with manual precedence in an Info popover", async () => {
    ...
    assetMocks.refreshCloudAssetRates.mockImplementationOnce(
      () =>
        new Promise((resolve) => {
    ...
    const refreshButton = await screen.findByTestId("refresh-rates-button");
    const infoButton = screen.getByRole("button", {
      name: "Thông tin về tỷ giá thủ công",
    });
    expect(
      screen.queryByText(/tỷ giá thủ công luôn được ưu tiên áp dụng/i),
    ...
    expect(assetMocks.refreshCloudAssetRates).toHaveBeenCalledWith(
      assetMocks.supabase,
    );
  ...
  it.each([
    ...
    },
    {
      ...
      role: "status",
      message: "Đã làm mới tất cả tỷ giá tự động.",
    },
    {
    ...
    "reports $name accurately",
    async ({ outcomes, role, message }) => {
      assetMocks.refreshCloudAssetRates.mockResolvedValueOnce(
        refreshResult(outcomes),
      );
      const user = userEvent.setup();
      renderScreen();

      await user.click(await screen.findByTestId("refresh-rates-button"));
  ...
  it("shows refresh failures and re-enables the refresh control", async () => {
    assetMocks.refreshCloudAssetRates.mockRejectedValueOnce(
      new Error("Dịch vụ tỷ giá tạm thời lỗi."),
    );
    const user = userEvent.setup();
    renderScreen();
  ...
  it("keeps a rate query error visible alongside refresh feedback", async () => {
    assetMocks.listCloudAssetRates.mockRejectedValue(
      new Error("Không thể tải danh sách tỷ giá."),
    );
    assetMocks.refreshCloudAssetRates.mockResolvedValueOnce(
      refreshResult({
        USD_VND: "cached",
    ...
    expect(
      await screen.findByText("Không thể tải danh sách tỷ giá."),
    ).toBeInTheDocument();
    await user.click(screen.getByTestId("refresh-rates-button"));

    expect(await screen.findByRole("status")).toHaveTextContent(
      "Tỷ giá tự động đã được cập nhật trước đó và vẫn còn hiệu lực.",
    );
    expect(
      screen.getByText("Không thể tải danh sách tỷ giá."),
    ).toBeInTheDocument();
  });
  ...
  it("preserves a typed override when refreshed query data changes", async () => {
    ...
    expect(
      await within(usdRate).findByText("1 USD = 26.000 ₫"),
    ).toBeInTheDocument();
  ...
  it("creates a cash account", async () => {
    ...
    await waitFor(() => {
      expect(assetMocks.upsertCloudAssetAccount).toHaveBeenCalledWith(
        assetMocks.supabase,
  ...
  it("preserves USD when editing a USD credit card", async () => {
    ...
    await user.click(
      await screen.findByRole("button", { name: "Sửa USD card" }),
    );
  ...
  it("preserves a negative overpaid credit-card balance when editing", async () => {
    ...
    await user.click(
      await screen.findByRole("button", { name: "Sửa Overpaid card" }),
    );
  ...
  it("requires a bank name before accepting an account identifier", async () => {
    ...
    await user.click(
      screen.getByRole("button", { name: "Tài khoản ngân hàng" }),
    );
  ...
  it("edits an account name and balance", async () => {
    ...
    await waitFor(() => {
      ...
      expect(
        assetMocks.upsertCloudAssetAccount.mock.calls.at(-1)?.[1],
  ...
  it("reorders accounts via move buttons", async () => {
    const user = userEvent.setup();
    assetMocks.accounts = [
    ...
    const moveDownBtn = await screen.findByRole("button", {
      name: /Đưa Ví tiền mặt xuống/i,
    });
  ...
  it("ignores drag and drop across asset groups", async () => {
    ...
    fireEvent.dragStart(
      within(cashRow).getByRole("button", { name: "Kéo Ví tiền mặt" }),
    );
  ...
  it("reorders interleaved accounts via move buttons without affecting other groups", async () => {
    const user = userEvent.setup();
    assetMocks.accounts = [
      account({ id: "cash-1", name: "Ví tiền mặt", sortOrder: 0 }),
      account({
