...
vi.mock("../../src/lib/date", async () => {
  const actual =
    await vi.importActual<typeof import("../../src/lib/date")>(
      "../../src/lib/date",
    );

  return {
    ...actual,
    todayVietnamDate: (now?: Date) =>
      now ? actual.todayVietnamDate(now) : dateHooks.today,
  };
});
...
describe("CalendarScreen", () => {
  it("renders the calendar inside a dark calendar panel", () => {
    ...
    expect(
      screen.getByRole("region", { name: /calendar month/i }),
    ).toBeInTheDocument();
  });
  ...
  it("loads the selected month and renders month totals plus expense day cells", () => {
    ...
    expect(cloudHooks.useMonthCloudTransactions).toHaveBeenCalledWith(
      "2026-07",
    );
    expect(
    ...
    expect(
      screen.getByRole("button", { name: /Select 2026-07-08/ }),
    ).toHaveAttribute("aria-pressed", "false");
    expect(
      screen.getByRole("region", { name: /transactions by date/i }),
    ).toBeInTheDocument();
  });
  ...
  it("locks the calendar header and day cells into seven columns", () => {
    ...
    const dayGrid = screen.getByRole("button", {
      name: /Select 2026-07-01/,
    }).parentElement;

    expect(weekdayGrid).toHaveStyle({
      gridTemplateColumns: "repeat(7, minmax(0, 1fr))",
    });
    expect(dayGrid).toHaveStyle({
      gridTemplateColumns: "repeat(7, minmax(0, 1fr))",
    });
  });
  ...
  it("selects the first transaction day when transactions arrive without committing stale empty day content", () => {
    ...
    const { rerender } = renderCalendarWithProbe(
    ...
    const staleEmptyCommit = snapshots.find(
      (snapshot) =>
    ...
    expect(
      screen.getByRole("article", { name: "15/06/2026 (Mon)" }),
    ).toBeInTheDocument();
    expect(
      within(
        screen.getByRole("article", { name: "15/06/2026 (Mon)" }),
      ).getByText("Food & Drinks"),
    ).toBeInTheDocument();
  });
  ...
  it("renders only selected-day transactions as newest-first tag links", async () => {
    ...
    expect(
      screen.queryByRole("article", { name: "07/07/2026 (Tue)" }),
  ...
  it("steps between months", async () => {
    ...
    expect(cloudHooks.useMonthCloudTransactions).toHaveBeenLastCalledWith(
      "2026-06",
    );
  ...
  it("uses the target month automatic date after month navigation without committing the previous manual date", async () => {
    ...
    expect(
      screen.getByRole("button", { name: /Select 2026-07-20/ }),
    ).toHaveAttribute("aria-pressed", "true");

    snapshots.length = 0;
    await user.click(screen.getByRole("button", { name: "Next month" }));

    const staleManualCommit = snapshots.find(
      (snapshot) =>
        snapshot.includes("August 2026") &&
        snapshot.includes("Select 2026-07-20"),
    );
    expect(staleManualCommit).toBeUndefined();
    expect(
      screen.getByRole("button", { name: /Select 2026-08-12/ }),
    ).toHaveAttribute("aria-pressed", "true");
    expect(
      within(
        screen.getByRole("article", { name: "12/08/2026 (Wed)" }),
      ).getByText("Food & Drinks"),
    ).toBeInTheDocument();
  });
  ...
  it("does not commit an empty target month from the previous hook state while the target month starts loading", async () => {
    ...
    expect(screen.getByText(/July 2026/i)).toBeInTheDocument();
    expect(
      screen.getByRole("button", { name: /Select 2026-07-10/ }),
    ).toHaveTextContent("25,000");

    snapshots.length = 0;
    await user.click(screen.getByRole("button", { name: "Next month" }));

    const staleEmptyTargetMonth = snapshots.find(
      (snapshot) =>
        snapshot.includes("August 2026") &&
        snapshot.includes("No transactions this month"),
    );
    ...
    expect(
      screen.queryByText("No transactions this month"),
    ).not.toBeInTheDocument();
    expect(screen.getByRole("status")).toHaveTextContent(
      "Loading transactions...",
    );
