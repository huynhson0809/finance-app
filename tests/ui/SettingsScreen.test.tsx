...
describe("SettingsScreen caps editor", () => {
  it("renders settings in grouped dark sections", async () => {
    ...
    expect(
      await screen.findByRole("heading", { name: /settings|cài đặt/i }),
    ).toBeInTheDocument();
    expect(
      screen.getByRole("region", { name: /language|ngôn ngữ/i }),
    ).toBeInTheDocument();
    expect(
      screen.getByRole("region", { name: /monthly budget|ngân sách/i }),
    ).toBeInTheDocument();
    expect(
      screen.getByRole("region", { name: /account|tài khoản/i }),
    ).toBeInTheDocument();
  });
  ...
  it("reveals email automation support details from a compact disclosure", async () => {
    ...
    const section = await screen.findByRole("region", {
      name: /email automation/i,
    });
    const infoButton = within(section).getByRole("button", {
      name: /show email automation details/i,
    });
  ...
  it("links to the report entry points from settings", async () => {
    ...
    for (const [name, href] of reportLinks) {
      expect(within(section).getByRole("link", { name })).toHaveAttribute(
        "href",
        href,
      );
    }
  ...
  it("saves the monthly budget total", async () => {
    ...
    await user.type(
      screen.getByLabelText(/monthly budget|ngân sách hàng tháng/i, {
        selector: "input",
      }),
      "6000000",
    );
  ...
  it("saves a per-category cap with save button", async () => {
    ...
    // open the disclosure
    fireEvent.click(
      await screen.findByRole("button", { name: /caps|hạng mục/i }),
    );
    const coffeeInput = await screen.findByLabelText(/coffee|cà phê/i);
    fireEvent.change(coffeeInput, { target: { value: "500000" } });
    const saveButtons = screen.getAllByRole("button", { name: /save|lưu/i });
  ...
  it("clears a cap when input is emptied and saved", async () => {
    await upsertBudget(currentVietnamMonth(), 5000000, {
      "coffee-bubble-tea": 500000,
    });
    render(
      <MemoryRouter>
        <SettingsScreen />
  ...
  it("keeps a newly saved monthly total when caps are saved separately", async () => {
    await upsertBudget(currentVietnamMonth(), 5000000);
    render(
      <MemoryRouter>
        <SettingsScreen />
    ...
    const budgetInput = await screen.findByLabelText(
      /monthly budget|ngân sách hàng tháng/i,
      { selector: "input" },
    );
    await waitFor(() => {
      expect(budgetInput).toHaveValue("5.000.000");
    });

    fireEvent.click(
      await screen.findByRole("button", { name: /caps|hạng mục/i }),
    );
    // Save new budget total first
    fireEvent.change(budgetInput, { target: { value: "6000000" } });
    const budgetSaveBtn = screen.getAllByRole("button", {
      name: /save|lưu/i,
    })[0];
    fireEvent.click(budgetSaveBtn);
    await waitFor(async () => {
      const b = await getBudgetForMonth(currentVietnamMonth());
      expect(b?.total).toBe(6000000);
    });
    ...
    const allSaveButtons = screen.getAllByRole("button", { name: /save|lưu/i });
    fireEvent.click(allSaveButtons[allSaveButtons.length - 1]);
...
describe("SettingsScreen account", () => {
  it("signs out from the account section", async () => {
    ...
    expect(
      await screen.findByRole("heading", { name: /account|tài khoản/i }),
    ).toBeInTheDocument();
    await user.click(
      screen.getByRole("button", { name: /sign out|đăng xuất/i }),
    );
  ...
  it("disables sign out while the request is pending", async () => {
    ...
    const pendingButton = await screen.findByRole("button", {
      name: /signing out|đang đăng xuất/i,
    });
  ...
  it("shows a visible error when sign out fails", async () => {
    authMocks.signOut.mockRejectedValue(
      new Error("Supabase rejected sign out"),
    );
    render(
      <MemoryRouter>
        <SettingsScreen />
