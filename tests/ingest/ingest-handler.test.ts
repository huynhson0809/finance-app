...
describe('createIngestTransactionHandler', () => {
  ...
  it.each([
    ...
    { label: 'empty', value: '' },
    { label: 'blank', value: '   ' },
  ])('reports a $label INGEST_SECRET as missing server config', async ({ value }) => {
    const { handle, createClient } = handler({ env: { INGEST_SECRET: value } });
  ...
  it('rejects invalid normalized payloads without calling the RPC', async () => {
    ...
    expect(response.status).toBe(400);
    expect(await jsonBody(response)).toEqual({ ok: false, error: 'invalid_bank' });
  ...
  it('passes a custom income Gemini category through to the RPC', async () => {
    const fetch = vi.fn(async (_input: string | URL | Request, init?: RequestInit) => {
      ...
      expect(payload.contents[0].parts[0].text)
        .toContain('custom-income-freelance-1234: Freelance');
  ...
  it('passes ACB credit alerts to the RPC as income transactions', async () => {
    ...
    expect(response.status).toBe(201);
    expect(await jsonBody(response)).toMatchObject({ ok: true, status: 'inserted' });
    expect(rpcCalls).toHaveLength(1);
    expect(rpcCalls[0]).toMatchObject({
      functionName: 'ingest_bank_email_transaction',
      args: {
        ...
        p_content: 'HUYNH NGOC SON CHUYEN TIEN GD 6189MSCBD2E4DZA8 080726-13:14:07',
  ...
  it('returns insert_failed when the RPC reports an error', async () => {
    ...
    expect(response.status).toBe(500);
    expect(await jsonBody(response)).toEqual({ ok: false, error: 'insert_failed' });
    expect(consoleError).toHaveBeenCalledWith('ingest transaction RPC failed', rpcError);
  });
