...
describe('transactions store', () => {
  ...
  it('lists transactions newest first', async () => {
    ...
    await addTransaction({ amount: 1, currency: 'VND', occurredAt: earlier, direction: 'expense', category: 'others', source: 'manual' });
    await addTransaction({ amount: 2, currency: 'VND', occurredAt: later, direction: 'expense', category: 'others', source: 'manual' });
  ...
  it('getTodayTotal sums today only', async () => {
    ...
    await addTransaction({ amount: 10000, currency: 'VND', occurredAt: today.toISOString(), direction: 'expense', category: 'others', source: 'manual' });
    await addTransaction({ amount: 99999, currency: 'VND', occurredAt: yesterday.toISOString(), direction: 'expense', category: 'others', source: 'manual' });
    expect(await getTodayTotal()).toBe(10000);
  });

  it('adds an income transaction locally', async () => {
    const t = await addTransaction({
      amount: 2_000_000,
      currency: 'VND',
      occurredAt: new Date().toISOString(),
  ...
  it('getTodayTotal ignores income but counts legacy rows as expense', async () => {
    const today = new Date(); today.setHours(12, 0, 0, 0);
    await addTransaction({
      amount: 10000,
      currency: 'VND',
    ...
    expect(await getTodayTotal()).toBe(17000);
  });
});
